...
st.markdown(
    ...
    /* Styling untuk Metric dengan NEON GLOW EFFECT */
    .metric-container { 
        text-align: center; 
        padding: 10px;
        margin-bottom: 20px;
    ...
    .metric-value { 
        ...
        font-weight: 900; 
        color: #ffffff; 
        /* Efek Neon Cyan - Bikin angka menyala */
        text-shadow: 0 0 10px rgba(126, 231, 214, 0.8), 
                     0 0 20px rgba(126, 231, 214, 0.6),
                     0 0 30px rgba(126, 231, 214, 0.4);
    }
...
with col_s1:
    model_choice = st.selectbox(
        "🔍 Pilih Versi Model YOLO",
        ["YOLO v8 Final", "YOLO v12 Final"]
    )
    
with col_s2:
    analysis_choice = st.selectbox(
        "⏱️ Granularitas Waktu Analisis",
...
if video_file:
    ...
    # Status processing dengan animasi spinner bawaan
    ...
    with col2:
        st.markdown(f"""
        <div class="metric-container">
            <div class="metric-value">{peak_traffic}</div>
            <div class="metric-label">Puncak ({analysis_choice})</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col3:
        st.markdown(f"""
        <div class="metric-container">
            <div class="metric-value">{avg_traffic}</div>
            <div class="metric-label">Rata-rata Volume</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with row2_col2:
        ...
        bar = alt.Chart(bar_df).mark_bar(cornerRadius=4).encode(
            ...
            y=alt.Y("type:N", sort='-x', title="Jenis"),
            color=alt.Color("type:N", legend=None, scale=alt.Scale(scheme="tealblues")),
            tooltip=["type", "count"]
    ...
    if len(df_processed) > 2: 
        ...
        scatter_chart = (
            ...
            .encode(
                ...
                color=alt.Color("cluster:N", scale=cluster_colors, title="Status"),
        ...
        with col_d2:
            with open(output_path, "rb") as f:
                st.download_button(
                    ...
                    f,
                    file_name="video_output_yolo.mp4",
                    use_container_width=True
                )

    # TAB 2: DATAFRAME (FIXED CMAP ERROR)
    with tab2:
        # Menggunakan cmap 'YlGnBu' (Valid) sebagai pengganti TealGrn
        st.dataframe(
            df_processed.style.background_gradient(cmap="YlGnBu", subset=["total"]), 
            use_container_width=True
        )

    # TAB 3: AUGMENTASI IMAGES
    with tab3:
